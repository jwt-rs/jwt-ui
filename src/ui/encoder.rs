...
fn draw_encoded_block<B: Backend>(f: &mut Frame<'_, B>, app: &App, area: Rect) {
  ...
  let block = layout_block_with_line(
    title_with_dual_style(" Encoded Token ".into(), title_hint.into()),
    app.light_theme,
    is_active,
  );
...
fn draw_signature_block<B: Backend>(f: &mut Frame<'_, B>, app: &App, area: Rect) {
  ...
  let is_active =
    app.data.decoder.blocks.get_active_route().active_block == ActiveBlock::DecoderSecret;
  let block = layout_block_with_line(
    title_with_dual_style(" Verify Signature ".into(), title_hint.into()),
    app.light_theme,
    is_active,
  );
